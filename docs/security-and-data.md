...
When `public/auth.md` or `public/.well-known/oauth-protected-resource` exists, `forge dev` serves them at `GET` and `HEAD` `/auth.md` plus `GET` and `HEAD` `/.well-known/oauth-protected-resource` so agents and authorization-aware clients can discover or probe the public authorization surface from the runtime.
