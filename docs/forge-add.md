...
`forge add auth workos` is the auth-provider form for the WorkOS recipe. It installs the WorkOS Node SDK, emits AuthKit/FGA generated artifacts, registers WorkOS secret names, writes a production-oriented `.env.example`, emits WorkOS-derived Forge policy templates, and returns Forge/WorkOS follow-up commands such as `forge workos install --json`, `forge workos install --yes --json`, `forge workos doctor --json`, `forge workos doctor --yes --json`, `forge workos seed --file workos-seed.yml --dry-run --json`, `forge workos seed --file workos-seed.yml --json`, and `forge workos prove --file workos-seed.yml --json`. If `@workos-inc/node` or `@workos-inc/authkit-react` is already declared in the relevant `package.json`, Forge skips that package-manager install, emits `FORGE_ADD_PACKAGE_ALREADY_DECLARED`, and still writes the generated adapter/AuthKit files. The WorkOS CLI remains responsible for authenticated hosted WorkOS changes; Forge validates the local app contract, claim mapping, seed file, webhook handler, webhook verifier, and FGA bridge before an agent applies those changes. Use `--dry-run` for validation-only seed checks; without `--dry-run`, Forge delegates to the authenticated WorkOS CLI and treats known duplicate-resource responses as an already-applied seed. Use `forge workos prove --file workos-seed.yml --json` as the no-dashboard aggregate proof before switching to `--real`.
...
| `workos-seed.yml` | Declarative WorkOS permissions, roles, resource types, orgs, redirect URIs, and CORS origins |
