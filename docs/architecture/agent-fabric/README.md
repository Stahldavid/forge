...
| [`S1.3_SCOPE_AND_GATE.md`](./S1.3_SCOPE_AND_GATE.md), [`S1.3_CONFORMANCE_PLAN.md`](./S1.3_CONFORMANCE_PLAN.md) | Adopted S1.3 Proof & Conformance planning package; S13-P08 complete. It authorized only the limited S1.3-A evidence/reproduction stage, not new tooling or P0b. |
...
| [`P0B_SCOPE_AND_GATE.md`](./P0B_SCOPE_AND_GATE.md) | Planning proposal for the first bounded nondeterministic P0b vertical: one real provider-backed model invocation behind the existing P0a authority/permit/result boundary. Not adopted until independent review and explicit merge. |
...
| S1.3 final closure | [PR #43](https://github.com/Stahldavid/forge/pull/43), `f29b717f496b7edcd74e3f230c962e568fd0e5d2` | `ADOPTED`; reviewed head `36a6c75dfcc2c59e33fb04a8dfb9ce95da10f2ed`; adopted tree `49677d9979c2652c96f1cc4e0f2f82a17cb10f8e` |
| P0b bounded model-adapter scope | pending independent review and explicit adoption | `PLANNING_PROPOSAL`; runtime implementation not yet authorized |
...
repository/reproduction baseline to `main@92c470ae2319aed9bde734d4506767677abe5a3c`; that
movement was assessed under S13-A01. S1.3-A was subsequently adopted by PR #19 at
`main@de9449648d821086db3d6f214c1a39cd4c659c7e`.

S1.3 is formally closed by PR #43 at
`main@f29b717f496b7edcd74e3f230c962e568fd0e5d2`. The closure preserves GAP-002 and GAP-008 as
historical/nonblocking limitations, closes GAP-001 and GAP-009 through separately adopted
evidence, and keeps GAP-003…007 within their previously adopted applicability boundaries.
Candidate/planning labels in exact historical source artifacts remain preserved as authored;
adoption merges and later supplements establish subsequent state.

The P0b Scope & Gate is a new planning proposal over that closed baseline. Until it is
independently reviewed and explicitly adopted, it does not authorize runtime implementation.
If adopted, it authorizes implementation work only for the bounded P0b-A model-invocation
vertical defined in that record; the implementation itself still requires separate evidence,
review and explicit merge.
...
change an `OP-*`/`I-*` obligation. The adopted S1.3-A baseline may classify gaps and
recommend a future decision, but it cannot itself implement a repair/tool or erase a
historical failed or inconclusive observation.

P0b planning inherits that precedence. A model/provider adapter may extend execution, but
cannot override the accepted deterministic authority/replay semantics merely because a live
provider behaves differently or exposes additional capabilities.
...
S1.2 — Reference Implementation Design is adopted. S1.3 planning and S1.3-A are adopted, and
S1.3 is now formally closed through the final closure record.

The closed S1.3 evidence package organizes evidence with stable IDs, separates historical
...
16 persisted event families, defines exact-SHA reproduction discipline and retains historical
limitations without converting them into false green claims.

A generic conformance runner, formal model checking, proof/evidence signing, cross-language
or JCS conformance, machine-readable evidence automation, production retention, production
persistence/recovery, consequential effects and the broader future Agent Fabric remain
outside the accepted P0a/S1.3 baseline.

The current P0b proposal is limited to one real provider-backed bounded external inference
behind the existing P0a permit/result boundary. It explicitly excludes tools/plugins,
consequential target mutation, production persistence/outbox, recovery epochs, adaptive
routing, persistent governed memory and plugin/evolution promotion. No P0b runtime behavior is
accepted until a separately reviewed and explicitly authorized implementation PR is merged.
