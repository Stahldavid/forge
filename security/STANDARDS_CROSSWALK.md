...
| Unsafe API consumption | `forge add`, package runtime guards, dependency API inspection | Partial | `forge deps api`, `forge deps runtime-compat` |
...
| Pre-publish security gate | `npm run forge -- security prove --json` in publish workflow | Covered |
| SBOM | Not emitted yet | Planned |
